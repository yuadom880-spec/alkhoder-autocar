...
-- عميل: إلغاء حجوزاته النشطة
DROP POLICY IF EXISTS "bookings_customer_update_own" ON bookings;
CREATE POLICY "bookings_customer_update_own" ON bookings
  FOR UPDATE TO authenticated
  USING (
    user_id = auth.uid()
...
-- عميل: حذف الحجوزات المنتهية/الملغاة/المرفوضة من قائمته
DROP POLICY IF EXISTS "bookings_customer_delete_own" ON bookings;
CREATE POLICY "bookings_customer_delete_own" ON bookings
  FOR DELETE TO authenticated
  USING (
    user_id = auth.uid()
...
-- ┌──────────────────────────────────────────────────────────────────────────┐
-- │ القسم 13ج: عروض السيارات حسب الفرع (وضع فرعي في الإدارة)                │
-- │ branch_offers: { "branch_uuid": { "daily": {...}, "monthly": {...} } }  │
-- └──────────────────────────────────────────────────────────────────────────┘

ALTER TABLE cars ADD COLUMN IF NOT EXISTS branch_offers JSONB NOT NULL DEFAULT '{}'::jsonb;
